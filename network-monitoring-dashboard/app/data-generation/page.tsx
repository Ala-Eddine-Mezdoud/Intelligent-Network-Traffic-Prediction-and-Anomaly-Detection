...
export default function DataGenerationPage() {
  ...
  const handleStart = async () => {
    try {
      ...
      setUiError(
        error.message ||
          "Failed to start capture. Make sure Mininet is running.",
      );
    } finally {
      setLoading(false);
    }
          ...
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription>
                  Historical GNN captures available for training
                </CardDescription>
              </div>
              <Button
                variant="outline"
            ...
            {datasets.length === 0 ? (
              ...
              <div className="space-y-4">
                {datasets.map((ds) => (
                  <div
                    key={ds.run_id}
                    className="p-4 border border-border rounded-lg flex items-start justify-between bg-card hover:bg-accent/5 transition-colors"
                  >
                        ...
                        <span className="mr-4">
                          ...
                          <strong>
                            {ds.summary?.duration_seconds
                              ? `${Math.round(ds.summary.duration_seconds / 60)}m`
                              : "0m"}
                          </strong>
                        </span>
                      </div>
